...
// --- RUTAS DE API ---
app.post('/login', async (req, res) => { try { const { username, password } = req.body; const user = await User.findOne({ username }); if (!user) return res.status(400).send('Usuario o contraseña incorrectos.'); const isMatch = await bcrypt.compare(password, user.password); if (!isMatch) return res.status(400).send('Usuario o contraseña incorrectos.'); req.session.user = { username: user.username, role: user.role }; console.log('SESIÓN CREADA PARA:', req.session.user); res.status(200).json({ message: 'Login exitoso', user: req.session.user }); } catch (err) { console.error('!!! ERROR EN LOGIN:', err); res.status(500).send('Error al iniciar sesión.'); } });
app.get('/api/user-status', (req, res) => { console.log('Revisando estado de usuario. Sesión encontrada:', req.session.user ? req.session.user : 'Ninguna'); if (req.session.user) { res.json({ loggedIn: true, user: req.session.user }); } else { res.json({ loggedIn: false }); } });
